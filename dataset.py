...
from utils.utils import set_all_seed, compress_images_jpeg
...
class paired_caption_adv_img_multi_insertion_dataset(Dataset):
    def __init__(self, ann_file, transform, image_root, max_words=MAX_WORDS, retrieval_mode_img='split', imgs_external_list=None, subset_size = float("inf"), seed_val=42, poison_insertion=5):
        loaded_ann = json.load(open(ann_file, 'r'))
        ...
        self.poison_insertion = poison_insertion
        ...
        for i, ann in enumerate(loaded_ann):
            ...
            for j, caption in enumerate(ann['caption']):
                ...
                candidates = [u for u in range(len(loaded_external_imgs_list)) if u != external_idx_image]
                img_adv_idxs = random.sample(candidates, min(self.poison_insertion, len(candidates)))
                img_adv_idx_paths = [self.external_image[idx] for idx in img_adv_idxs]
                self.adv_img_internal[txt_id] = [
                    self.image_dict.get(path, None) for path in img_adv_idx_paths
                ]
                self.txt2adv_img[txt_id] = img_adv_idxs
    ...
    def __getitem__(self, final_index):
        ...
        adv_image_idxs = self.txt2adv_img[index]
        adv_image_paths = [os.path.join(self.image_root, self.external_image[i]) for i in adv_image_idxs]
        adv_images = [self.transform(Image.open(p).convert("RGB")) for p in adv_image_paths]
        adv_images_internal = self.adv_img_internal[index]

        return text, ret_texts, true_image, adv_images, index, ret_text_idxs, true_image_idx, adv_image_idxs, adv_images_internal, true_image_name

    def collate_fn(self, batch):
        txt_groups, ret_txt_groups, true_imgs, adv_imgs_groups, txt_ids, ret_txt_ids, true_imgs_idx, adv_imgs_idx_groups, adv_imgs_internal_groups, true_image_names = list(zip(*batch))
        true_imgs = torch.stack(true_imgs, 0)
        adv_imgs_list = [torch.stack(adv_imgs, 0) for adv_imgs in adv_imgs_groups]
        return txt_groups, ret_txt_groups, true_imgs, adv_imgs_list, list(txt_ids), list(ret_txt_ids), list(true_imgs_idx), list(adv_imgs_idx_groups), list(adv_imgs_internal_groups), true_image_names

class img_dataset(Dataset):
    def __init__(self, ann_file, transform, image_root, jpg_quality=None):
        loaded_ann = json.load(open(ann_file, 'r'))
    ...
    def __getitem__(self, index):
        ...
        if self.jpg_quality is not None:
            image = compress_images_jpeg(image, self.jpg_quality)
