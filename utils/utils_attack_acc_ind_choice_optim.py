...
from utils.utils import compress_images_jpeg, set_all_seed
...
from utils.utils_common_attack import IMAGE_ATTACK_DICT, TEXT_SIMILARITY_DICT, SIMAttacker, save_configurations_to_json

from utils.utils_statistic import IndependentAccuracyAttackManager

from utils.utils_common_attack_acc import retrieval_accuracy_for_attack_choice, build_adv_sims, build_adv_sims_itm
...
def retrieval_attack_acc_ind_choice_optim(model, m_tokenizer, ref_model, tokenizer, data_loader, img_preprocess, clean_sims_dict, clean_feats_dict, device, args, config, path_save_results, top_k=10):
    model.eval()
    ...
    if jpg_quality is not None:
        path_results_adv = os.path.join(path_save_results, f"jpg_quality:{jpg_quality}", f"model:{model_name}", "valuations", f"adversarial_ind_choice_accuracy_attack-on_qt:{EVAL_ON_QT}_optim", f"dataset:{dataset_name}", f"seed:{args.seed}", f"{subset_size_val}", f"(config::{config_attack_upperpath_str})", f"{config_attack_exp_name_str}.{date}_{uuid}")
    else:
        path_results_adv = os.path.join(path_save_results, f"model:{model_name}", "valuations", f"adversarial_ind_choice_accuracy_attack-on_qt:{EVAL_ON_QT}_optim", f"dataset:{dataset_name}", f"seed:{args.seed}", f"{subset_size_val}", f"(config::{config_attack_upperpath_str})", f"{config_attack_exp_name_str}.{date}_{uuid}")
    ...
    num_all_sample = len(data_loader.dataset)
    num_to_pois = args.num_to_pois
    if num_to_pois is not None:
        idx_to_pois = random.sample(range(num_all_sample), num_to_pois)
    else:
        idx_to_pois = list(range(num_all_sample))
    idx_remains = [idx for idx in range(num_all_sample) if idx not in idx_to_pois]
    num_to_eval = args.num_to_eval
    if num_to_eval is not None:
        idx_to_eval_all = random.sample(idx_remains, num_to_eval)
    else:
        idx_to_eval_all = list(range(num_all_sample))
    
    num_sel_dict = {
        "num_all_sample": num_all_sample,
        "num_to_pois": num_to_pois,
        "idx_to_pois": idx_to_pois,
        "idx_remains": idx_remains,
        "num_to_eval": num_to_eval,
        "idx_to_eval_all": idx_to_eval_all
    }

    clean_accuracy_attack_results_dict_all = []
    len_clean_results_acc_all = []
    for idx_pois in idx_to_pois:
        idx_to_eval = [x for x in idx_to_eval_all if x != idx_pois]
        clean_accuracy_results = retrieval_accuracy_for_attack_choice(data_loader, clean_sims_dict, args, config, idx_to_eval, top_k=10, clean_val=True, on_qt=EVAL_ON_QT)
        clean_accuracy_attack_results_dict = clean_accuracy_results["accuracy"]
        clean_accuracy_attack_results_dict_all.append(clean_accuracy_attack_results_dict)
        len_clean_results_acc = len(clean_accuracy_results["accuracy_list"])
        len_clean_results_acc_all.append(len_clean_results_acc)

    accuracy_attack_manager = IndependentAccuracyAttackManager(
        args, 
        config_retrieval,
        config_attack,
    ...
    for batch_idx, (captions_group, ret_captions_list_group, true_images, pois_images, captions_ids, ret_captions_list_group_ids, true_images_ids, pois_images_ids, pois_images_internal_ids, true_images_names) in enumerate(tqdm(data_loader)):
        ...
        for idx in range(pois_images.shape[0]):

            if all_idx in idx_to_pois:

                images_adv, texts_args, texts_init = attacker.sim_attack(pois_images[idx].unsqueeze(0), [captions_group[idx]], num_iters=config_attack['image_attack_num_iter'], true_images=true_images[idx].unsqueeze(0), images_names=[true_images_names[idx]], indexes_captions=[captions_ids[idx]])
                ...
                if config["use_itm"] and model_name == "BLIP-2":
                    adv_sims_dict = build_adv_sims_itm(model, clean_feats_dict, adv_poisoning_image_dict)
                else:
                    adv_sims_dict = build_adv_sims(clean_feats_dict, image_adv_feat)
                idx_to_eval = [x for x in idx_to_eval_all if x != all_idx]
                adv_accuracy_results = retrieval_accuracy_for_attack_choice(data_loader, adv_sims_dict, args, config, idx_to_eval, top_k=10, on_qt=EVAL_ON_QT)
                adv_accuracy_attack_results_dict = adv_accuracy_results["accuracy"]
                len_adv_results_acc = len(adv_accuracy_results["accuracy_list"])

                accuracy_attack_manager.add_attack_result(
                    caption=caption,
                    caption_id=caption_id,
                    ...
                    text_init=text_init,
                    texts_arg=texts_arg,
                    adv_accuracy_attack_results_dict=adv_accuracy_attack_results_dict,
    ...
    accuracy_attack_manager.set_time(total_time)

    accuracy_attack_manager.compute_averages()

    adv_results = accuracy_attack_manager.get_results()

    path_adv_results = os.path.join(path_results_adv, "adv_acc_results.json")
...
def eval_attack_acc_ind_choice_optim(model, m_tokenizer, ref_model, tokenizer, data_loader, img_preprocess, clean_sims_dict, clean_feats_dict, device, args, config, path_save_results):
    ...
    retrieval_attack_acc_ind_choice_optim(model, m_tokenizer, ref_model, tokenizer, data_loader, img_preprocess, clean_sims_dict, clean_feats_dict, device, args, config, path_save_results)