...
def retrieval_attack_acc_ind_choice_batch(model, m_tokenizer, ref_model, tokenizer, data_loader, img_preprocess, clean_sims_dict, clean_feats_dict, device, args, config, path_save_results, top_k=10):
    model.eval()
    ...
    if jpg_quality is not None:
        path_results_adv = os.path.join(path_save_results, f"jpg_quality:{jpg_quality}", f"model:{model_name}", "valuations", f"adversarial_ind_choice_accuracy_attack-on_qt:{EVAL_ON_QT}_batch", f"dataset:{dataset_name}", f"seed:{args.seed}", f"{subset_size_val}", f"(config::{config_attack_upperpath_str})", f"{config_attack_exp_name_str}.{date}_{uuid}")
    else:
        path_results_adv = os.path.join(path_save_results, f"model:{model_name}", "valuations", f"adversarial_ind_choice_accuracy_attack-on_qt:{EVAL_ON_QT}_batch", f"dataset:{dataset_name}", f"seed:{args.seed}", f"{subset_size_val}", f"(config::{config_attack_upperpath_str})", f"{config_attack_exp_name_str}.{date}_{uuid}")
...
def eval_attack_acc_ind_choice_batch(model, m_tokenizer, ref_model, tokenizer, data_loader, img_preprocess, clean_sims_dict, clean_feats_dict, device, args, config, path_save_results):
    ...
    retrieval_attack_acc_ind_choice_batch(model, m_tokenizer, ref_model, tokenizer, data_loader, img_preprocess, clean_sims_dict, clean_feats_dict, device, args, config, path_save_results)