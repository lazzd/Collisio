...
from utils.utils_common_attack import IMAGE_ATTACK_DICT, TEXT_SIMILARITY_DICT, SIMAttacker, retrieval_eval_top, retrieval_eval_itm_top, add_in_ir_1_top_k_results_dict_all, compute_means_for_each_key, save_configurations_to_json


def retrieval_attack_top(model, m_tokenizer, ref_model, tokenizer, data_loader, img_preprocess, clean_sims_dict, device, args, config, path_save_results, top_k=10):
    model.eval()
    ...
    if jpg_quality is not None:
        path_results_adv = os.path.join(path_save_results, f"jpg_quality:{jpg_quality}", f"model:{model_name}", "valuations", "adversarial_top", f"dataset:{dataset_name}", f"seed:{args.seed}", f"{subset_size_val}", f"(config::{config_attack_upperpath_str})", f"{config_attack_exp_name_str}.{date}_{uuid}")
    else:
        path_results_adv = os.path.join(path_save_results, f"model:{model_name}", "valuations", "adversarial_top", f"dataset:{dataset_name}", f"seed:{args.seed}", f"{subset_size_val}", f"(config::{config_attack_upperpath_str})", f"{config_attack_exp_name_str}.{date}_{uuid}")
    ...
    poison_insertion = args.poison_insertion

    start_time = time.time()

    for batch_idx, (captions_group, ret_captions_list_group, true_images, pois_images_list, captions_ids, ret_captions_list_group_ids, true_images_ids, pois_images_ids_list, pois_images_internal_ids_list, true_images_names) in enumerate(tqdm(data_loader)):
        print(f'--------------------> batch:{batch_idx}/{len(data_loader)}')

        pois_images_list_batches = [
            torch.stack([sample[i] for sample in pois_images_list], dim=0)
            for i in range(poison_insertion)
        ]

        num_elem_batch = len(pois_images_ids_list)
        complete_images_adv_lists = [[] for _ in range(num_elem_batch)]

        for pois_images in pois_images_list_batches:
            ...
            for i_in, img_adv_in in enumerate(images_adv):
                complete_images_adv_lists[i_in].append(img_adv_in)

        images_adv_sample_list = [torch.stack(adv_imgs, 0) for adv_imgs in complete_images_adv_lists]

        for idx, images_adv_sample in enumerate(images_adv_sample_list):

            # ---------------------------------
            # Embeds images_adv
            # ---------------------------------
            with torch.no_grad():
                images_adv_sample_norm = img_preprocess(images_adv_sample).to(device)
                images_adv_sample_output = model.inference_image(images_adv_sample_norm)
            ...
            # ----------
            images_adv_sample_feat = images_adv_sample_output['image_feat'].float().detach()
            # ----------
            pois_images = pois_images_list[idx]
            pois_images_ids = pois_images_ids_list[idx]
            pois_images_internal_ids = pois_images_internal_ids_list[idx]
            ...
            # ----------
            # ADV POISONING IMAGE
            adv_poisoning_image_dict_sample_list = []

            for idx_in_sample, image_adv_feat in enumerate(images_adv_sample_feat):
                # ----------
                # ADV POISONING IMAGE
                ...
                if model_name == 'BLIP-2':
                    adv_poisoning_image_dict["image_embed"] = images_adv_sample_output['image_feat'][idx_in_sample].float().detach()
                    adv_poisoning_image_dict["vit_feat"] = images_adv_sample_output['vit_feat'][idx_in_sample].float().detach()
                # ----------
                adv_poisoning_image_dict_sample_list.append(adv_poisoning_image_dict)
        
            # ----------
            clean_poisoning_image_dict_sample_list = []

            for pois_image_inner, pois_image_internal_id in zip(pois_images, pois_images_internal_ids):
                pois_image = pois_image_inner.unsqueeze(0)
                # ----------
                ...
                if pois_image_internal_id is None:
                    ...
                    # ----------
                    clean_poisoning_image_dict_sample_list.append(clean_poisoning_image_dict)
            ...
            for pois_image_id in pois_images_ids:
                ...
                adv_image_name_list.append(adv_image_name)
            
            pois_ids_str = ".".join(str(i) for i in pois_images_ids)
            adv_image_name_tensor_dict = f"TxtID-{caption_id}_ImgTrue-{true_image_id}_ImgsAdv-{pois_ids_str}.pt"
            # ----------
            ...
            adv_tensor_dict = {
                ...
                "texts_arg": texts_arg,
                "ret_captions": [],
                "poison_insertion": poison_insertion,
            }
            ...
            for ret_idx in range(len(ret_captions_list)):
                ...
                ir_1_top_k_results_dict, rank_results_dict = retrieval_eval_top(clean_sims_dict, clean_poisoning_image_dict_sample_list, adv_poisoning_image_dict_sample_list, ret_caption_id, ret_idx, true_image_id, pois_images_internal_ids, top_k=top_k)
                ...
                if config["use_itm"] and model_name == "BLIP-2":
                    ir_1_top_k_itm_results_dict, rank_itm_results_dict = retrieval_eval_itm_top(model, clean_sims_dict, clean_poisoning_image_dict_sample_list, adv_poisoning_image_dict_sample_list, ret_caption_id, true_image_id, pois_images_internal_ids, device, k_test=128, batch_size=128, top_k=top_k)
            ...
            for adv_image_name, adv_image_tensor, pois_image in zip(adv_image_name_list, images_adv_sample, pois_images):
                path_adv_image = os.path.join(path_results_adv_img, adv_image_name)
                adv_image = to_pil(adv_image_tensor)
                ...
                try:
                    ...
                    adv_img  = adv_image_tensor.squeeze(0).detach().cpu()
                    ...
                    # grayscale
                    pert_mag = delta.abs().amax(dim=0)  # HxW
    ...
    path_adv_results = os.path.join(path_results_adv, "adv_top_results.json")
...
def eval_attack_top(model, m_tokenizer, ref_model, tokenizer, data_loader, img_preprocess, clean_sims_dict, device, args, config, path_save_results):
    ...
    retrieval_attack_top(model, m_tokenizer, ref_model, tokenizer, data_loader, img_preprocess, clean_sims_dict, device, args, config, path_save_results)