...
from models.fare_model.ext_methods import FAREExtMethods
...
def load_model(model_name, model_ckpt, text_encoder, config, device, quant_source_model=None):
    ...
    ### load checkpoint
    ...
    elif model_name in ["FARE2", "FARE4"]:
        if model_name == "FARE2":
        ...
        if hasattr(model.visual, 'image_size'):
            model.visual.input_resolution = model.visual.image_size[0]

        model.__class__ = type('FAREExtended', (model.__class__, FAREExtMethods), {})
