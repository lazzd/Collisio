...
def clean_eval(model, m_tokenizer, test_transform, img_preprocess, device, args, config, path_save_results, external_imgs=False):
    ...
    if jpg_quality is not None:
        path_clean_eval_model_dataset = os.path.join(path_save_results, f"jpg_quality:{jpg_quality}", f"model:{model_name}", "clean_files", f"dataset:{dataset_name}")
    else:
    ...
    if (args.retrieval_mode_img == 'all') or (external_imgs and args.retrieval_mode_target_img == 'all'):
        test_img_dataset = img_dataset(config_retrieval['img_list_file'], test_transform, config_retrieval['image_root'], jpg_quality=jpg_quality)
    else:
        test_img_dataset = img_dataset(config_retrieval['test_file'], test_transform, config_retrieval['image_root'], jpg_quality=jpg_quality)
...
def load_clean_sims_t2i(args, config, path_results, external_imgs=False):
    ...
    if jpg_quality is not None:
        path_clean_eval_model_dataset = os.path.join(path_results, f"jpg_quality:{jpg_quality}", f"model:{model_name}", "clean_files", f"dataset:{dataset_name}")
    else:
...
def load_clean_feats(args, config, path_results, external_imgs=False):
    model_name = get_model_name(args)
    dataset_name = get_dataset_name_clean_files(args, config, external_imgs=external_imgs)
    ...
    if model_name == 'BLIP-2':
        path_clean_image_feats = os.path.join(path_clean_eval_model_dataset, "clean_image_features_dict.pt")
        clean_image_features_dict = torch.load(path_clean_image_feats)
        feats_dict["clean_image_features_dict"] = clean_image_features_dict

        path_clean_text_feats = os.path.join(path_clean_eval_model_dataset, "clean_text_features_dict.pt")
        clean_text_features_dict = torch.load(path_clean_text_feats)
        feats_dict["clean_text_features_dict"] = clean_text_features_dict
    else:
        path_clean_text_feats = os.path.join(path_clean_eval_model_dataset, "clean_text_feats.pt")
        clean_text_feats = torch.load(path_clean_text_feats)
        feats_dict["clean_text_feats"] = clean_text_feats

        path_clean_image_feats = os.path.join(path_clean_eval_model_dataset, "clean_image_feats.pt")
        clean_image_feats = torch.load(path_clean_image_feats)
        feats_dict["clean_image_feats"] = clean_image_feats
