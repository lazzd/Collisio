...
def compute_sim_matrix(clean_image_feats, clean_text_feats, vit_feats, text_embeds, text_ids, text_atts, k_test, model, device, batch_size, top_sim_k_cpu=False):
    sims_matrix = []
    ...
    for i, sims in tqdm(enumerate(sims_matrix), total=len(sims_matrix), desc="Processing Text-to-Image Similarity"):
        ...
        for batch_start in range(0, k_test, batch_size):
            ...
            if top_sim_k_cpu:
                score_matrix_t2i[i, topk_idx_batch] = score.cpu() + topk_sim[batch_start:batch_end].cpu()
            else:
