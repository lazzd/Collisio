...
from dataset import paired_caption_adv_img_multi_insertion_dataset
...
from utils.utils_attack_top import eval_attack_top
...
def main(args, config):
    ...
    test_dataset = paired_caption_adv_img_multi_insertion_dataset(config_retrieval['test_file'], s_test_transform, config_retrieval['image_root'], retrieval_mode_img=args.retrieval_mode_img, imgs_external_list=imgs_external_list, subset_size=args.subset_size, seed_val=args.seed, poison_insertion=args.poison_insertion)
    test_loader = DataLoader(test_dataset, batch_size=args.batch_size, num_workers=4, collate_fn=test_dataset.collate_fn)

    eval_attack_top(model, m_tokenizer, ref_model, tokenizer, test_loader, images_normalize, clean_sims_dict, device, args, config, PATH_EXP)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--poison_insertion', default=5, type=int)
