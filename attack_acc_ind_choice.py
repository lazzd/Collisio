...
from utils.utils_clean import clean_eval, load_clean_sims_t2i, load_clean_feats
from utils.utils_attack_acc_ind_choice_optim import eval_attack_acc_ind_choice_optim
from utils.utils_attack_acc_ind_choice_batch import eval_attack_acc_ind_choice_batch
...
def main(args, config):
    ...
    clean_feats_dict = load_clean_feats(args, config, PATH_EXP)
    ...
    if args.optim:
        eval_attack_acc_ind_choice_optim(model, m_tokenizer, ref_model, tokenizer, test_loader, images_normalize, clean_sims_dict, clean_feats_dict, device, args, config, PATH_EXP)
    else:
        eval_attack_acc_ind_choice_batch(model, m_tokenizer, ref_model, tokenizer, test_loader, images_normalize, clean_sims_dict, clean_feats_dict, device, args, config, PATH_EXP)

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--config_retrieval', default='./configs/Retrieval_flickr_server.yaml')
    parser.add_argument('--config_attack', default='./configs/syn/absolute_configs_4-255/ret:1/aug:exact/choice:random/config_attack_exp1.yaml')
    ...
    parser.add_argument('--source_model', default='ViT-B/16', type=str, choices=['ViT-B/16', 'RN101', 'BLIP-2', 'FARE2', 'FARE4'])
    ...
    parser.add_argument('--jpg_quality', default=None, type=int, help="If set, quality of the jpg.")
    
    parser.add_argument('--num_to_pois', default=None, type=int)
    parser.add_argument('--num_to_eval', default=None, type=int)
    parser.add_argument('--optim', default=True, type=bool, help="Execute with velocity optimization.")
    ...
    with open(args.config_attack, 'r') as config_file:
        config_attack = yaml.load(config_file)
